#!/usr/bin/env bash
# Paired 1+1 test for the clean LAP6 + SEC284, no-VLM route.
# This script intentionally passes no LAP8 checkpoint and uses only the
# Stage-1 checkpoint's `lap` state for the latent-action path.
set -euo pipefail
...
SEC284_CKPT=${SEC284_CKPT:-"$ROOT/outputs/sec284_frozen_expert_behavior_kd_2000step/step-002000.pt"}
STATE_STATS="$ROOT/cache/lap_stage1_task14/state_stats.json"
EXPERT_CKPT=${EXPERT_CKPT:-"$ROOT/cache/lap8_phase1_official_action_expert.pt"}
...
OUT_ROOT=${OUT_ROOT:-"$ROOT/results/eval_runs/sec284_behavior_kd_step2000_lap6_no_vlm_paired_seed0_1x"}
...
run_split() {
    ...
    "$SIM_PY" "$ROOT/examples/Robotwin/eval_files/robotwin_batch_bridge.py" \
        ...
        --task_name "$TASK" --task_config "$task_config" --ckpt_setting "lap6_sec284_no_vlm_${label}" --seed 0 \
