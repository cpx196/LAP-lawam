#!/usr/bin/env bash
# Real-VLM LAP6 control with SEC284 shadow traces, clean + randomized seed 0.
set -euo pipefail
...
SEC284_CKPT="$ROOT/outputs/sec284_frozen_expert_behavior_kd_2000step/step-002000.pt"
STATE_STATS="$ROOT/cache/lap_stage1_task14/state_stats.json"
OUT_ROOT=${OUT_ROOT:-"$ROOT/results/eval_runs/sec284_step2000_shadow_trace_seed0_1x"}
TASK=move_pillbottle_pad
SERVER_GPU=${SERVER_GPU:-3}
SIM_GPU=${SIM_GPU:-5}
PORT=${PORT:-11053}
...
run_split() {
    ...
    CUDA_VISIBLE_DEVICES="$SERVER_GPU" "$SERVER_PY" \
        -m deployment.model_server.server_policy_lap6_sec284_shadow_trace \
        --host 0.0.0.0 --port "$PORT" --idle-timeout -1 \
        --ckpt-path "$POLICY_CKPT" \
        --lap-checkpoint "$LAP6_CKPT" --lap-state-stats "$STATE_STATS" \
        --sec284-checkpoint "$SEC284_CKPT" --trace-dir "$run_root/traces" \
    ...
    for _ in $(seq 1 240); do
        ...
        if ! kill -0 "$server_pid" 2>/dev/null; then
            tail -n 120 "$run_root/server.log" >&2 || true
            exit 1
        fi
        sleep 2
    done
    ...
    "$SIM_PY" "$ROOT/examples/Robotwin/eval_files/robotwin_batch_bridge.py" \
        ...
        --ckpt_setting "lap6_vlm_sec284_shadow_${label}" --seed 0 \
