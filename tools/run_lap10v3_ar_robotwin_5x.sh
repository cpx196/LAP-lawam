#!/usr/bin/env bash
# LAP10V3 AR checkpoint: 5 clean + 5 randomized RoboTwin evaluation, FP32.
set -euo pipefail
...
AR_CKPT=${AR_CKPT:-"$ROOT/outputs/lap10v3_ar_expert_task14_2000step/lap10v3_ar_step0002000.pt"}
REPLAN_STEPS=${REPLAN_STEPS:-36}
TEST_NUM=${TEST_NUM:-5}
OUT_ROOT=${OUT_ROOT:-"$ROOT/results/eval_runs/lap10v3_ar_2000step_5x_clean_random_fp32"}
...
CUDA_VISIBLE_DEVICES="$SERVER_GPU" "$SERVER_PY" -m deployment.model_server.server_policy_lap8_no_vlm \
    --ckpt-path "$POLICY_CKPT" --joint-checkpoint "$AR_CKPT" \
...
run_split() {
    ...
    "$SIM_PY" "$ROOT/examples/Robotwin/eval_files/robotwin_batch_bridge.py" \
        ...
        --ckpt_setting "$(basename "$AR_CKPT" .pt)_${label}" --seed 0 \
